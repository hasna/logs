...
function getErrorsByMessage(
  ...
  `)
    .all({ $p: projectId, $since: since, $until: until }) as {
    message: string;
    service: string | null;
    count: number;
  }[];
...
export function compare(
  ...
  `)
    .all({
      ...
      $bs: bSince,
      $bu: bUntil,
    }) as CompareResult["perf_delta_by_page"];
