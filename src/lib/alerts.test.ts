...
describe("alert rules CRUD", () => {
  ...
  it("updates a rule", () => {
    ...
    const updated = updateAlertRule(db, rule.id, {
      enabled: 0,
      threshold_count: 99,
    });
...
describe("alert evaluation", () => {
  it("does not fire when under threshold", async () => {
    ...
    // No throw = passes
    await expect(
      evaluateAlerts(db, p.id, null, "error"),
  ...
  it("fires when threshold exceeded (log action)", async () => {
    ...
    const rule = db
      .prepare("SELECT last_fired_at FROM alert_rules WHERE project_id = ?")
      .get(p.id) as { last_fired_at: string | null };
