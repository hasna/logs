...
import { rotateByProject, rotateLogs } from "./rotate.ts";
...
describe("rotateByProject", () => {
  it("only rotates logs for the specified project", () => {
    const db = createTestDb();
    const p1 = db
      .prepare("INSERT INTO projects (name) VALUES ('p1') RETURNING id")
      .get() as { id: string };
    const p2 = db
      .prepare("INSERT INTO projects (name) VALUES ('p2') RETURNING id")
      .get() as { id: string };
    ingestBatch(
      db,
      Array.from({ length: 8 }, () => ({
