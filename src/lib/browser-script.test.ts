...
describe("getBrowserScript", () => {
  ...
  it("uses a scoped browser token when provided on the script tag", () => {
    const script = getBrowserScript("http://localhost:3460");
    expect(script).toContain("data-browser-token");
    expect(script).toContain("data-write-token");
    expect(script).toContain("X-Logs-Browser-Token");
  });
});
