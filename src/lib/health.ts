...
export function getHealth(db: Database): HealthResult {
  ...
  const scheduler_jobs = (
    db
      .prepare("SELECT COUNT(*) as c FROM scan_jobs WHERE enabled = 1")
      .get() as { c: number }
  ).c;
  const open_issues = (
    db
      .prepare("SELECT COUNT(*) as c FROM issues WHERE status = 'open'")
      .get() as { c: number }
  ).c;
  ...
  const logs_by_level = Object.fromEntries(
    levelRows.map((r) => [r.level, r.c]),
  );

  const oldest = db.prepare("SELECT MIN(timestamp) as t FROM logs").get() as {
    t: string | null;
  };
  const newest = db.prepare("SELECT MAX(timestamp) as t FROM logs").get() as {
    t: string | null;
  };
