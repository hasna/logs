...
export function createJob(
  db: Database,
  data: { project_id: string; schedule: string; page_id?: string },
): ScanJob {
  return db
    .prepare(`
...
export function listJobs(db: Database, projectId?: string): ScanJob[] {
  if (projectId) {
    return db
      .prepare(
        "SELECT * FROM scan_jobs WHERE project_id = $p ORDER BY created_at DESC",
      )
      .all(sqlBindings({ $p: projectId })) as ScanJob[];
  }
  return db
    .prepare("SELECT * FROM scan_jobs ORDER BY created_at DESC")
    .all() as ScanJob[];
}

export function getJob(db: Database, id: string): ScanJob | null {
  return db
    .prepare("SELECT * FROM scan_jobs WHERE id = $id")
    .get(sqlBindings({ $id: id })) as ScanJob | null;
}

export function updateJob(
  db: Database,
  id: string,
  data: { enabled?: number; schedule?: string; last_run_at?: string },
): ScanJob | null {
  ...
  params.$id = id;
  return db
    .prepare(`UPDATE scan_jobs SET ${fields} WHERE id = $id RETURNING *`)
    .get(sqlBindings(params)) as ScanJob | null;
}

export function deleteJob(db: Database, id: string): void {
  db.prepare("DELETE FROM scan_jobs WHERE id = $id").run(
    sqlBindings({ $id: id }),
  );
}
...
export function createScanRun(
  db: Database,
  data: { job_id: string; page_id?: string },
): ScanRun {
  return db
    .prepare(`
  ...
  `)
    .get(
      sqlBindings({ $job_id: data.job_id, $page_id: data.page_id ?? null }),
    ) as ScanRun;
}
...
export function listScanRuns(
  ...
  return db
    .prepare(
      "SELECT * FROM scan_runs WHERE job_id = $j ORDER BY started_at DESC LIMIT $l",
    )
    .all(sqlBindings({ $j: jobId, $l: limit })) as ScanRun[];
}
