...
export function getLatestSnapshot(
  ...
  if (pageId) {
    return db
      .prepare(
        "SELECT * FROM performance_snapshots WHERE project_id = $p AND page_id = $pg ORDER BY timestamp DESC LIMIT 1",
      )
      .get(
        sqlBindings({ $p: projectId, $pg: pageId }),
      ) as PerformanceSnapshot | null;
  }
  return db
    .prepare(
      "SELECT * FROM performance_snapshots WHERE project_id = $p ORDER BY timestamp DESC LIMIT 1",
    )
    .get(sqlBindings({ $p: projectId })) as PerformanceSnapshot | null;
}
...
export function getPerfTrend(
  ...
  return db
    .prepare(
      `SELECT * FROM performance_snapshots WHERE ${conditions.join(" AND ")} ORDER BY timestamp DESC LIMIT $limit`,
    )
    .all(sqlBindings(params)) as PerformanceSnapshot[];
}
