...
export async function runLighthouse(
  ...
  try {
    ...
    try {
      ...
      return {
        lcp: (audits["largest-contentful-paint"]?.numericValue ?? null) as
          | number
          | null,
        fcp: (audits["first-contentful-paint"]?.numericValue ?? null) as
          | number
          | null,
        cls: (audits["cumulative-layout-shift"]?.numericValue ?? null) as
          | number
          | null,
        tti: (audits.interactive?.numericValue ?? null) as number | null,
        ttfb: (audits["server-response-time"]?.numericValue ?? null) as
          | number
          | null,
