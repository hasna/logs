...
export function listAlertRules(db: Database, projectId?: string): AlertRule[] {
  if (projectId) {
    return db
      .prepare(
        "SELECT * FROM alert_rules WHERE project_id = $p ORDER BY created_at DESC",
      )
      .all(sqlBindings({ $p: projectId })) as AlertRule[];
  }
  return db
...
export function updateAlertRule(
  ...
  data: Partial<
    Pick<
      AlertRule,
      "enabled" | "threshold_count" | "window_seconds" | "webhook_url"
    >
  >,
): AlertRule | null {
  ...
  if (!fields)
    return db
      .prepare("SELECT * FROM alert_rules WHERE id = $id")
      .get(sqlBindings({ $id: id })) as AlertRule | null;
  const params = Object.fromEntries(
    Object.entries(data).map(([k, v]) => [`$${k}`, v]),
  );
  params.$id = id;
  return db
    .prepare(`UPDATE alert_rules SET ${fields} WHERE id = $id RETURNING *`)
    .get(sqlBindings(params)) as AlertRule | null;
}

export function deleteAlertRule(db: Database, id: string): void {
  db.prepare("DELETE FROM alert_rules WHERE id = $id").run(
    sqlBindings({ $id: id }),
  );
}
...
export async function evaluateAlerts(
  ...
  `)
    .all(
      sqlBindings({ $p: projectId, $level: level, $service: service }),
    ) as AlertRule[];

  for (const rule of rules) {
    const since = new Date(
      Date.now() - rule.window_seconds * 1000,
    ).toISOString();
    const conditions = [
      "project_id = $p",
      "level = $level",
    ...
    const { count } = db
      ...
      .get(sqlBindings(params)) as { count: number };
...
async function fireAlert(
  ...
  db.prepare(
    "UPDATE alert_rules SET last_fired_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = $id",
  ).run(sqlBindings({ $id: rule.id }));
