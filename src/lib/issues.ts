...
export function listIssues(
  ...
  return db
    .prepare(
      `SELECT * FROM issues ${where} ORDER BY last_seen DESC LIMIT $limit`,
    )
    .all(sqlBindings(params)) as Issue[];
}

export function getIssue(db: Database, id: string): Issue | null {
  return db
    .prepare("SELECT * FROM issues WHERE id = $id")
    .get(sqlBindings({ $id: id })) as Issue | null;
}
...
export function updateIssueStatus(
  ...
  return db
    .prepare("UPDATE issues SET status = $status WHERE id = $id RETURNING *")
    .get(sqlBindings({ $id: id, $status: status })) as Issue | null;
}
