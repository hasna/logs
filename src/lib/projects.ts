...
export function getProject(db: Database, id: string): Project | null {
  return db
    ...
    .get(sqlBindings({ $id: id })) as Project | null;
}
...
export function updateProject(
  ...
  params.$id = id;
  return db
    .prepare(`UPDATE projects SET ${fields} WHERE id = $id RETURNING *`)
    .get(sqlBindings(params)) as Project | null;
}

// Pages
export function createPage(
  db: Database,
  data: { project_id: string; url: string; path?: string; name?: string },
): Page {
  return db
    .prepare(`
...
export function listPages(db: Database, projectId: string): Page[] {
  return db
    .prepare(
      "SELECT * FROM pages WHERE project_id = $p ORDER BY created_at ASC",
    )
    .all(sqlBindings({ $p: projectId })) as Page[];
}

export function getPage(db: Database, id: string): Page | null {
  return db
    .prepare("SELECT * FROM pages WHERE id = $id")
    .get(sqlBindings({ $id: id })) as Page | null;
}
...
export function resolveProjectId(
  ...
  const p = db
    .prepare("SELECT id FROM projects WHERE LOWER(name) = LOWER($n)")
    .get(sqlBindings({ $n: idOrName })) as { id: string } | null;
  return p?.id ?? null;
}

export function touchPage(db: Database, id: string): void {
  db.prepare(
    "UPDATE pages SET last_scanned_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = $id",
  ).run(sqlBindings({ $id: id }));
}
