...
import { createScanRun, finishScanRun, listJobs, updateJob } from "./jobs.ts";
...
export function startScheduler(db: Database): void {
  ...
  cron.schedule("0 * * * *", () => {
    ...
    if (result.deleted > 0)
      console.log(
        `Retention: deleted ${result.deleted} logs across ${result.projects} project(s)`,
      );
  });
