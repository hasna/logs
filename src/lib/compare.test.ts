...
describe("compare", () => {
  it("detects new errors in window B", () => {
    ...
    // Window A: old error
    db.prepare(
      "INSERT INTO logs (project_id, level, message, service, timestamp) VALUES (?, 'error', 'old bug', 'api', ?)",
    ).run(p.id, dayAgo);
    // Window B: new error
    db.prepare(
      "INSERT INTO logs (project_id, level, message, service, timestamp) VALUES (?, 'error', 'new bug', 'api', ?)",
    ).run(p.id, now);

    const result = compare(db, p.id, dayAgo, halfDayAgo, halfDayAgo, now);
    expect(result.new_errors.some((e) => e.message === "new bug")).toBe(true);
    expect(result.resolved_errors.some((e) => e.message === "old bug")).toBe(
      true,
    );
  });
