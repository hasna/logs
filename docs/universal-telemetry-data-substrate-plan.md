...
Latest local evidence captured on 2026-06-18:
...
- `bun test`: 345 passing tests and 3974 expectations across 40 files after the bounded multi-process segment rotation/rebuild, rebuild-lock, process-level crash/write-error recovery, cross-surface redaction canary regressions, rowid catch-up-before-bus stream regression, first real-life validation lab harness, stream load/resume validation slice, first OTLP JSON bridge, hardened first structured logging bridge, JSONL follow-mode structured-log validation, hard `--max-lines` cap regression, split-UTF-8 follow-mode regression, SDK Pino/Winston structured transport helper regressions, final transport shutdown flush regressions, Winston legacy-wrapper/stream compatibility regressions, real installed logger package validation, bounded SDK structured-transport retry/drop regressions, active in-flight batch protection under queue pressure, per-record retry exhaustion, stable transport-generated retry event IDs across rebatching, opt-in redacted SDK file-spool replay after restart with redacted send-context preservation, secret-bearing batch-prefix redaction, malformed/unsupported spool-line skipping while valid records still replay, load-time spool overflow drop reporting, opt-in browser universal-event `localStorage` spooling with overlap serialization, post-failure retry redaction, failed in-flight batch preservation at capacity, corrupt/invalid-only cleanup, browser-token route compatibility, browser listener cleanup, browser console/error/rejection/fetch capture, browser navigation/resource timing/Web Vitals capture with capped Web Vital observer shutdown, opt-in browser/Node fetch traceparent propagation with invalid, uppercase-value-invalid, version-invalid, duplicate existing-header preservation, browser no-cors suppression, same-origin non-HTTP/blob suppression, absolute target exact-origin matching, explicit CORS-enabled cross-origin trace target, preflight, and unlisted-target validation in the real-browser lab, browser relative fetch URL canonicalization, browser relative collector recursion prevention, browser runtime capture opt-out flags, first bounded `logs run` process resource usage metric coverage, first bounded `logs run` child process-tree telemetry coverage, first bounded `logs run` build artifact metadata coverage, artifact path redaction coverage, nested artifact-body projection coverage, bounded source-map validation projection coverage, source-map fallback-ID, scalar-smuggling, top-level-attribute, content-hash, raw-container, and root-body live/rebuild parity regressions, first bounded `logs run` JUnit XML test-report metadata coverage, dedicated SQLite test-report/test-case projection metadata allowlisting, and first API/CLI/MCP test-report query surfaces.
...
- `bunx biome check scripts/browser-spool-validation-lab.ts src/lib/sdk-client.test.ts`: passes for the real-browser validation lab and focused SDK client regressions covering browser spool replay/cleanup, post-failure retry redaction, overlapping flush serialization, and browser-token identity stripping. Broader Biome checks over SDK source still hit pre-existing whole-file formatting/import/lint diagnostics.
...
- Repo-wide `bun run lint` still fails; current Biome output reports 172 existing errors across `src/` files, mostly formatting/import-order and unsafe template-literal suggestions.
